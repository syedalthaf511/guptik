...
class AddBoardScanScreen extends StatefulWidget {
  ...
  const AddBoardScanScreen({super.key, required this.homeId, this.roomId});
...
class _AddBoardScanScreenState extends State<AddBoardScanScreen> {
    ...
    if (status.isGranted) {
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Location permission is required to scan for Wi-Fi.'),
          ),
        );
      }
    }
  ...
  // 4. Send HTTP request to ESP12-E with WiFi credentials + owner/home/room IDs
  Future<void> _sendCredentialsToBoard() async {
    ...
    try {
      ...
      // Force the phone to route HTTP traffic over the ESP's Wi-Fi network
      ...
      final response = await http
          .post(
            ...
            body: {
              's': _ssidController.text.trim(), // WiFi SSID (user-provided)
              'p': _passwordController.text
                  .trim(), // WiFi password (user-provided)
              'o': user.id, // Owner ID (auto)
              'h': widget.homeId, // Home ID (auto)
              'r': widget.roomId ?? '', // Room ID (auto)
            },
          )
          .timeout(const Duration(seconds: 10));

      // ALWAYS release forced Wi-Fi
      await WiFiForIoTPlugin.forceWifiUsage(false);

      if (response.statusCode == 200) {
        // Disconnect from ESP so phone returns to normal internet
        ...
        if (mounted) {
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text(
                'Board configured! It will register automatically.',
              ),
            ),
          );
        }
        ...
        throw Exception(
          'ESP rejected credentials (Status: ${response.statusCode})',
        );
      }
    } catch (e) {
      await WiFiForIoTPlugin.forceWifiUsage(false);
      ...
      if (mounted) {
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(SnackBar(content: Text('Failed to provision: $e')));
      }
    }
  }
      ...
      appBar: AppBar(
        ...
        actions: [
          IconButton(
            ...
            onPressed: (_isScanning || _connectedBoard != null)
                ? null
                : _requestPermissionsAndScan,
          ),
        ],
